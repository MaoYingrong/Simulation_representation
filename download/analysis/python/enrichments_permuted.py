...
def prepare_associations_universe(target, disease):
    """Creates a ficticial set of associations based on the combination of all possible targets and diseases."""
    return (
        target.selectExpr("id as targetId")
        .crossJoin(disease.selectExpr("id as diseaseId"))
        .select(
            "*",
            F.lit("random").alias("datasourceId"),
            F.lit("random").alias("datatypeId"),
        )
    )
...
def run_analysis(
    conf,
    seed,
    stratify_therapeutic_area,
):
    """Run enrichment analysis."""
    # Load raw datasets
    evidence = spark.read.parquet(conf.default.input.evidence_path)
    disease = spark.read.parquet(conf.default.input.disease_path).persist()
    target = spark.read.parquet(conf.default.input.target_path).persist()
    ...
    universe_size = associations.select("targetId", "diseaseId").distinct().count()
    random_associations = (
        prepare_associations_universe(target, disease)
        .sample(fraction=1.0, seed=seed)
        .limit(universe_size)
        .withColumn("seed", F.lit(seed))
    )
    ...
    clinical = (
        ...
        .join(random_associations, on=["targetId", "diseaseId"], how="left")
    )
...
def main(
    ...
    """Generate enrichment reports on 100 random permutations of the data."""
    ...
    for i in range(100):
        ...
        analysis_result = run_analysis(
            conf=conf, seed=i, stratify_therapeutic_area=stratify_therapeutic_area
        )
        analysis_result.write.parquet(
            f"{conf.default.permuted_enrichments_template}/{stratify_therapeutic_area.value}_seed_{i}",
            mode="overwrite",
        )
